...
class Qwen2:
    ...
    def __init__(self, model_path: Union[str, Path]):
        ...
        if not self._native.create(config):
            raise RuntimeError("Failed to create model in C++ backend")
        
        # 3. 加载所有权重到C++后端
        self._load_all_weights(model_path)
        
        print(f"Qwen2 model loaded to C++ backend")
        print(f"  Layers: {config.get('num_hidden_layers', 24)}")
        print(f"  Hidden size: {config.get('hidden_size', 1536)}")
        print(f"  Vocab size: {config.get('vocab_size', 151936)}")
    ...
    def _load_weight_file(self, file_path: Path):
        ...
        # 使用safetensors直接读取数据
        # 注意：这里不使用PyTorch或其他框架
        with safetensors.safe_open(str(file_path), framework="pt", device="cpu") as f:
            for key in f.keys():
                # 直接获取numpy数组（safetensors支持）
                try:
                    # safetensors可以直接返回numpy数组
                    ...
                    elif np_array.dtype == np.bfloat16:
                        # 转换为float32
                        np_array = np_array.astype(np.float32)

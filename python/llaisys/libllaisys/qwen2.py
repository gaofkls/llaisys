...
class Qwen2Native:
    ...
    def create(self, meta_dict: dict) -> bool:
        """创建C++模型实例"""
        # 创建meta结构体
        meta = LlaisysQwen2Meta()
        
        # 填充meta数据
        meta.dtype = 0  # 默认float32，根据你的DataType枚举
        meta.nlayer = meta_dict.get("num_hidden_layers", 24)
        meta.hs = meta_dict.get("hidden_size", 1536)
        meta.nh = meta_dict.get("num_attention_heads", 24)
        meta.nkvh = meta_dict.get("num_key_value_heads", 8)
        meta.dh = meta_dict.get("head_dim", meta.hs // meta.nh)
        meta.di = meta_dict.get("intermediate_size", 5504)
        meta.maxseq = meta_dict.get("max_position_embeddings", 32768)
        meta.voc = meta_dict.get("vocab_size", 151936)
        meta.epsilon = meta_dict.get("rms_norm_eps", 1e-6)
        meta.theta = meta_dict.get("rope_theta", 1000000.0)
        meta.end_token = meta_dict.get("eos_token_id", 151643)
        ...
        if hasattr(LIB_LLAISYS, 'llaisysQwen2ModelCreate'):
            self.model_ptr = LIB_LLAISYS.llaisysQwen2ModelCreate(
                meta, DeviceType.CPU, None, 0
            )
        else:
            # 使用dummy指针用于测试
            print("Warning: Using dummy Qwen2 model for testing")
            self.model_ptr = ctypes.c_void_p(1)
        
        return self.model_ptr is not None and self.model_ptr.value is not None
